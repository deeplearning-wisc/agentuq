...
class UserSimulator(BaseUser):
    ...
    def _generate_next_message(
        ...
        Args:
            ...
            max_empty_retries: Maximum number of retries when the LLM returns an empty response.
        ...
        for attempt in range(max_empty_retries):
            ...
            if assistant_message.has_text_content() or assistant_message.is_tool_call():
                break
            logger.warning(
                f"User simulator LLM returned empty response (attempt {attempt + 1}/{max_empty_retries}). Retrying..."
            )
        if not (
            assistant_message.has_text_content() or assistant_message.is_tool_call()
        ):
            logger.error(
                f"User simulator LLM returned empty response after {max_empty_retries} retries."
            )
